...
- Header wordmark is a breadcrumb reinforcing the `.co` play and doubling as back-nav:
  ...
  - **Made crumb** (Sept 2026): a project that's for sale (shop on, `shop_status: available`) reads `humancrafted.co/made/<slug>` instead — `data-wm-state="made"`; the whole `humancrafted.co/` is the dim home link (one piece, so ≤1024px drops it and shows `made/<slug>`), `made` links to `/made/`, slug current. Same `.wm-new` wipe.
...
- "Let's co/lab" pill shows on all pages except `/lab/` and shop pages (nav pill on desktop, floating FAB on phones).
...
- **Shelved experiment — floating co/lab + the bag (Sept 2026):** saved on the local branch **`experiment/made-bag-floating-colab`** (commit `0ce6e3f`, not pushed). It had co/lab as a floating pill at every width (footer-lift script, footer padding sized to the pill) so the cart sat beside Made, and "the bag comes out of Made": hovering the pill opened the empty cart's collapsed slot (pure CSS via a `--nav-gap` variable), sliding the pill left to its /made/ position while the bag popped into place, plus an arrival hop on `/made/` (`assets/js/made-bag.js`). Jon pulled it back: the pill moving left was odd UX and the whole thing felt fiddly. `git show 0ce6e3f` / `git checkout experiment/made-bag-floating-colab` to revisit.
- **Finding Made — nav pill, `/made/`, price pills (Sept 2026):** prompted by a visitor who couldn't find anything to buy — clicking Made in the filter bar isn't discoverable, and "shop" can't be the label (here it means the workshop). Three pieces:
  - **Nav pill:** `.made-nav-btn` in `_layouts/default.html`, first in the nav, before the co/lab pill: the Made wordmark on an *outlined* pill (inset 2px ring), same 2.5rem height as co/lab so they pair, filled ink with `aria-current` on `/made/`. **Hover** (and `:focus-visible`, not on `/made/`): the co/lab accent fill, nothing else — a slide-out "for sale →" label was tried and dropped. **No co/lab on shop pages** (`body.shop-page`: `/made/`, products for sale, cart, thanks) at any width — nav pill and FAB both hidden in `shop.css`. The filter-bar Made pill keeps its own lift hover. Only while the shop is on and ≥1 project is available. Stays in the nav on phones, where co/lab goes to the FAB — but on the **homepage only** — any page with a breadcrumb wordmark hides it (crumb + pill + icons overran 375px; under Made the crumb already says made). Phone fit (375px) is tight: with the pill, `.site-nav` gap drops to 1.25rem; home/co-re with a non-empty cart (five items) squeezes further via `:has()` rules; and every crumb page hides the pill on phones (see above). Styles in `shop.css`. Umami: `Made nav`.
  - **`/made/`** (`made.md`, no longer a redirect): the homepage grid and nothing else — no hero, no Projects header or filter bar, `hide_title: true` (new default.html flag; the crumb names the page). The grid lives in `_includes/project-grid.html`, shared with `index.md` (`made=true` filters to `shop_status: available`), so the two can't drift. Wordmark has a `/made/` branch: `humancrafted.co/made`, its own `data-wm-state="made-hub"` so the ≤1024px/phone rules that drop the `humancrafted.co/` prefix from `made` crumbs don't apply — a lone "made" looked odd and had no way home. `shop: true` (cart icon shows empty), `shop_page: true` (withheld when shop off). Empty state is PLACEHOLDER copy.
  - **Homepage Made filter stays** (Jon, 2026-09-24): Made products should read as part of the whole body of work, not a separate thing. `?category=made` still filters the homepage. The crumb's `made` and core.md's "buy some" now go to `/made/`; the project page's Categories "made" link still goes to the filter (it sits among the other category links).
  - **Card pill = price** (`_includes/for-sale-icon.html`, takes `project=`): lowest in-stock variant price, "from $X" if in-stock prices differ, whole dollars when cents are zero, "sold out" (muted outline) if every variant is at 0. Replaced the Made wordmark on cards — the nav pill now teaches the brand. Cards still carry `data-shop="available"`. Pill history: "$" disc → "shop" → "for sale" → wordmark → price.
- **Homepage Made filter:** the filter bar has a **Made filter** right after "all" — the Made wordmark (`_includes/made-wordmark.html`) on a pill, outlined at rest / ink when active — with `data-filter="made"` and an aria-label, only while the shop is on and ≥1 project is available; the inline script's `made` branch is Liquid-guarded so an off build carries no trace. `?category=made` works like any category link; unknown filters fall back to "all". Available project pages lead their "Categories:" line with a plain-text **"made"** link (`.meta-link--for-sale`) into that filter — text, not the mark, because an image among underlined mono words reads as a sticker. Pill height is 1.625rem so the mark's strokes stay ≥ ~2px. A `made.humancrafted.co` subdomain would be a redirect rule on the hcd-checkout Vercel project.
