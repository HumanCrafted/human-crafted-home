...
#### Tooling Catalog
The [[tooling|Tooling]] note is a catalogue of every bit in the shop across three categories: CNC bits (tag `bit`), hand router bits (tag `router-bit`), and drill bits (tag `drill-bit`) — each its own table on the page, rendered straight from `site.docs`. Same shape as the coffee notes: structured front matter drives both the public table and Obsidian's own view.
...
- Each bit carries a short sharpie serial — a letter plus a sequence number, so a loose bit in a drawer is identifiable without calipers. CNC bits encode the one property you can't see by looking: **U**pcut, **D**owncut, **C**ompression, **V**-bit, **B**allnose, **S**traight, **R**oundover. Hand router bits (**H**) and drill bits (**T**, for Twist) are flat series instead — a hand-router profile is visible on sight, and a twist drill's geometry doesn't vary bit to bit the way CNC end mills do, so the letter just marks the category
- Feeds and speeds live in a table in each CNC bit's body and mirror the presets stored on that tool in the Fusion **hub** library (`hub://Human Crafted`), so the site and the CAM software agree. Fusion's own API can read and write that library but cannot create new ones — `importToolLibrary` fails on the current build, so libraries are made in the Fusion UI and populated from there. Hand router bits and drill bits skip Fusion entirely — they don't run in the CNC, so there's no tool library entry or feed table, just the vault note
- `tooling.base` gives Obsidian its own views: all CNC bits, grouped by diameter, grouped by geometry, a *Needs Attention* filter for anything not `in service`, a *Reorder* view carrying vendor, part number, price, and purchase link, and dedicated *Router Bits* / *Drill Bits* views
