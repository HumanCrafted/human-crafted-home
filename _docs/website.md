...
- US state lines are drawn from a vendored GeoJSON (`assets/data/us-states.json`, 50 states, ~88KB) rather than relying on Positron's baked-in tile lines, which are too faint to read — fetched at runtime (static geometry, not a Jekyll collection) and styled from `--foreground` so it restyles with the marker MutationObserver on theme flip. `interactive: false` keeps it decorative — clicks and popups pass straight through to the pins and tiles
