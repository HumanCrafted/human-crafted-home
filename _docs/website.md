...
#### Tooling Catalog
The [[tooling|Tooling]] note is a catalogue of every bit in the shop across four categories: CNC bits (tag `bit`), hand router bits (tag `router-bit`), drill bits (tag `drill-bit`), and hole cutters (tag `hole-cutter`) — each its own table on the page, rendered straight from `site.docs`. Same shape as the coffee notes: structured front matter drives both the public table and Obsidian's own view.
...
- Each bit carries a short sharpie serial — a letter plus a sequence number, so a loose bit in a drawer is identifiable without calipers. CNC bits encode the one property you can't see by looking: **U**pcut, **D**owncut, **C**ompression, **V**-bit, **B**allnose, **S**traight, **R**oundover. Hand router bits (**H**), drill bits (**T**, for Twist), and hole cutters (**K**) are flat series instead — a hand-router profile is visible on sight, a twist drill's geometry doesn't vary bit to bit the way CNC end mills do, and a hole cutter needed a letter at all once every word-initial option (H/T/C/S) was already spoken for — so the letter just marks the category
- Feeds and speeds live in a table in each CNC bit's body and mirror the presets stored on that tool in the Fusion **hub** library (`hub://Human Crafted`), so the site and the CAM software agree. Fusion's own API can read and write that library but cannot create new ones — `importToolLibrary` fails on the current build, so libraries are made in the Fusion UI and populated from there. Hand router bits, drill bits, and hole cutters all skip Fusion entirely — none of them run in the CNC, so there's no tool library entry or feed table, just the vault note
- `tooling.base` gives Obsidian its own views: all CNC bits, grouped by diameter, grouped by geometry, a *Needs Attention* filter for anything not `in service`, a *Reorder* view carrying vendor, part number, price, and purchase link, and dedicated *Router Bits* / *Drill Bits* / *Hole Cutters* views
